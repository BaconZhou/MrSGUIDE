    ...
    namespace Tree {
        ...
        mat GiTree::boostrapCI(const arma::mat &numX, const arma::imat &catX,
            ...
            for (int bi = 0; bi < bootNum; bi++) {
                // Rcpp::checkUserInterrupt();
                arma::mat gammaU(K, this->yp, arma::fill::zeros),
                thetaU(K, this->yp, arma::fill::ones);
                ...
                node *bootRoot = buildTree(numXBoot, catXBoot, YBoot, TrtBoot, 1, 1);
                
                this->crossValidation(bootRoot, numXBoot, catXBoot, YBoot, TrtBoot,
                ...
                gamma += gammaU / (double)(TNodeSize * (tp));
                theta += thetaU;
                // if ((bi > 0) && (bi % 10 == 0)) logger->info("Finish {} bootstrap samples", bi);
                if ((bi > 0) && (bi % 10 == 0)) cout << "finish " << bi << " bootstrap samples." << endl;
            }
            gamma /= (double)bootNum;
            theta /= (double)bootNum;
            // logger->debug("Gamma: \n{}\n Theta: {}\n", gamma, theta);
            return GetBootAlpha(gamma, theta, alpha, alphaK);
        }
