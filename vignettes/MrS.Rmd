...
The original [GUIDE](http://pages.stat.wisc.edu/~loh/guide.html) algorithm is developed by Professor Wei-Yin Loh, which has more variates of features. `MrSGUIDE` only use `GUIDE` `Gi` for subgroup identification for single and multiple responses. If you like `MrSGUIDE` and want to explore more, please try [GUIDE](http://pages.stat.wisc.edu/~loh/guide.html)!

The general analysis pipline is as follows:
...
`MrSGUIDE` can be installed from Github repository "BaconZhou/MrS" now. And users need to download and install `devtool` first from CRAN by the following command:
...
\begin{align*}
    y_1 &= x_1 + x_2 + 2I(Z=1)I(\text{gender}==\text{Female}) + \epsilon_1 \\
    y_2 &= x_1 + x_3 + 2I(Z=1)I(\text{gender}==\text{Female}) + \epsilon_2 \\
...
Specifically, `numX` contains all numerical features. `gender` and `country` are two categorical features. `z` is the binary treatment assignment and `y1` and `y2` are two responses. `train` is the data frame contains. `role` is a vector provide the role of each column.
...
`mrsobj` is the reture object from `MrSFit()`, it could then be passed to other functions in the `MrSGUIDE` package. For example `printTree()` will print the tree result. 

```{r quick-example-print}
printTree(mrsobj = mrsobj)
```

You can also set `detail = FALSE` to hide the detial.

```{r quick-example-print-detailF}
printTree(mrsobj = mrsobj, details = F)
```

`MrSGUIDE` also provide `plotTree()` function use `visNetwork` package. The return object is a list contains three items:

1. A tree result `treePlot`
2. A data frame used for plotting `nodeTreat`
3. A node statistic plot `trtPlot`
...
```{r, engine='bash', echo=FALSE, comment=NULL, results='hide'}
latex test.tex
dvips test.dvi
ps2pdf test.ps
rm test.tex test.aux test.dvi test.log test.ps
```

The PDF file is shown below.

```{r, echo=FALSE, fig.width=20, fig.align='center', fig.pos="h"}
include_graphics(path = './test.pdf')
```


This quick example aimed at provide a general understanding of what `MrSGUIDE` is capable of doing. In the rest of the chapter, we will show more intruction about how to use the function step by step.
...
In general, we recommand run `bootNum = 1000` to calibrate the confidence interval of treatment effect estimators. But it will take more times. For illustration purpose, here we set `bootNum = 50` and `alpha = 0.05`.
...
The tree depth is controled by `maxDepth` with default value `maxDepth = 5`. You can also change `maxDepth = 1` with only a root node.
...
`minTrt` controls inside a node, how many data should each treatment levels contains. For example, if we set `minTrt = 30`, the terminal nodes of resulting tree have both 30 observationas in `Z=0` and `Z=1`
...
`MrSFit()` uses cross-validataion for tree pruning. `CVFolds` controls the number of cross-validation, and `CVSE` is a parameters control the error allowance. If we set `CVFolds == 0`, `MrSFit()` will not perform cross-validation for pruning.
...
In general, we recommand setting `CVSE=0.5` and `CVFolds=10`, a smaller `CVSE` will provide a larger tree result.
