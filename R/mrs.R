...
#' 
#' @param nX numeric X matrix
#' @param cX categorical X matrix, must recode as integer
#' @param Y multiple response matrix
#' @param Trt Treatment matrix
#' @param bestK step wise K
#' @param maxDepth maximum tree depth
#' @param minTrt minimal treatment 
#' @param minData minimal data in each  node
#' @param batchnum minmal bathch number to search in split, default is 1
#' @param CVFolds cross validataion times
#' @param CVSE cross validation SE
#' @param filename tree save name
#' @export
mrs <- function(dataframe, role, bestK = 1, 
                        maxDepth = 5, 
    ...
    t2 = Sys.time()
    stopifnot(NROW(Y) > 0)
    treeRes = mrs.gi.pure(nX = nX, cX = cXL$intX, 
                     Y = Y, Trt = TrtL$intX, 
...
mrs.gi.pure <- function(nX, cX, Y, Trt, 
